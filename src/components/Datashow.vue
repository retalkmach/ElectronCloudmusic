<template>
  <div id="songs">
    <ul v-if="songsData.length != 0" @click.right="toggleMenuShow">
      <li
  ...
  methods: {
    playMusic(index: number) {
      ...
      for (let i = 0; i < this.songsData.length; i++) {
        ...
        const song = {
          id: this.songsData[i].id,
          name: this.songsData[i].name,
          artists,
        };
        for (let j = 0; j < this.songsData[i].ar.length; j++) {
          song.artists.push(j < this.songsData[i].ar[j].name);
        }
    ...
    addPlaylist(){
      let songdata:any = {
        id: this.songsData[this.chooseIndex].id,
        name: this.songsData[this.chooseIndex].name,
        artist:[]
      }
      for(let i=0;i<this.songsData[this.chooseIndex].ar.length;i++){
        songdata.artist.push(this.songsData[this.chooseIndex].ar[i].name)
      }
